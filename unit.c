...
#define LEX(str, fun) (unit_lex_t){.p_lex = str, .length = sizeof(str), .p_fun = (lex_parse_func)fun}
...
unit_parse_result_t unit_parse_section(mini_t *p_mini, unit_t *p_unit, const char *const p_section, unit_service_t *p_services, size_t *p_service_cnt)
{
	unit_parse_result_t rc = UNIT_PARSE_OKAY;
	...
	for (size_t i = 0; i < p_unit->services.size; ++i)
	{
		switch(p_unit->services.elem[i].sdscope)
		{
			case UNIT_SCOPE_SYSTEM:
				...
				break;
			default:
				LOG_WNG("Unsupported scope=%d for service '%s' of '%s', ignoring.",
